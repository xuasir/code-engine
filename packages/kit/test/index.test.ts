...
describe('kit exports', () => {
  it('根入口仅暴露 module API', () => {
    ...
    expect(kit.createArtifactsBuilder).toBeTypeOf('function')
    expect(kit.SlotPresets).toBeTypeOf('object')
    ...
    expect((kit as any).createGenerateBuilder).toBeUndefined()
    expect((kit as any).defineGenerateUnit).toBeUndefined()
    expect((kit as any).registerGenerateHost).toBeUndefined()
  ...
  it('internal 子路径暴露内核 API', () => {
    ...
    expect(internal.createArtifactsRuntime).toBeTypeOf('function')
    expect(internal.snapshotArtifactsRegistry).toBeTypeOf('function')
  })
})
